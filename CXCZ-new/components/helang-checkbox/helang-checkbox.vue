<template>
	<view class="container" :class="column">
		<view v-for="(v,i) in list" :key="i">
			<view class="item" :class="{ 'active':(type=='radio' && index == i) || (type=='checkbox' && v.checked) }"
				:data-i="i" @tap="change">{{v.value}}</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			set(data) {
				...
				this.column = (data.column in column) ? column[data.column] : 'col_5';
